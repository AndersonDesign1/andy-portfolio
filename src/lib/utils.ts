...
export function getStaggeredAnimation(index: number, stagger: number = 0.1): string {
...
export function validateMinLength(value: string, minLength: number): string | null {
  return value.trim().length >= minLength ? null : `Must be at least ${minLength} characters`
}
