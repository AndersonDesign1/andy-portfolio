...
export async function submitGiveawayFeedback(formData: FormData) {
  ...
  try {
    await resend.emails.send({
      ...
      subject: `📝 New Giveaway Feedback from ${name || email}`,
      replyTo: email,
      html: `
        <div style="font-family: system-ui, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background: #f4f4f5; padding: 20px; border-radius: 8px; margin-top: 20px;">
            ...
            <p style="white-space: pre-wrap;">${brandDescription}</p>
            
            <p><strong>Desired Features:</strong></p>
            <p style="white-space: pre-wrap;">${features}</p>
            
            <p><strong>Domain Status:</strong> ${domainStatus}</p>
